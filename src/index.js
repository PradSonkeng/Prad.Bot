...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    h1 { font-size: 24px; font-weight: 700; color: #25d366; margin-bottom: 4px; }
    .version { font-size: 13px; color: #555; margin-bottom: 32px; }
    #waiting { display: flex; flex-direction: column; align-items: center; gap: 12px; }
    .spinner { width: 48px; height: 48px; border: 4px solid #222; border-top-color: #25d366; border-radius: 50%; animation: spin 0.8s linear infinite; }
    @keyframes spin { to { transform: rotate(360deg); } }
    #qr-box { display: none; flex-direction: column; align-items: center; gap: 16px; }
    #qr-box img { width: 260px; height: 260px; border-radius: 16px; border: 4px solid #25d366; padding: 8px; background: #fff; }
    .instructions { background: #1a1a1a; border-radius: 12px; padding: 16px; text-align: left; font-size: 13px; color: #aaa; line-height: 1.8; width: 100%; }
    .instructions span { color: #25d366; font-weight: 600; }
    .timer { font-size: 13px; color: #555; }
    .timer b { color: #f59e0b; }
    #connected-box { display: none; flex-direction: column; align-items: center; gap: 16px; }
    .check { width: 80px; height: 80px; background: #25d366; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 40px; animation: pop 0.4s ease; }
    @keyframes pop { 0% { transform: scale(0); } 70% { transform: scale(1.2); } 100% { transform: scale(1); } }
    .connected-text { font-size: 20px; font-weight: 700; color: #25d366; }
    .connected-sub { font-size: 14px; color: #666; }
    .badge { background: #1a2e1a; border: 1px solid #25d366; border-radius: 20px; padding: 6px 16px; font-size: 13px; color: #25d366; }
    .uptime { font-size: 12px; color: #666; margin-top: 16px; }
  </style>
</head>
<body>
  <div class="card">
  ...
  <script>
    let countdown = 60, timer = null, uptimeInterval = null;
    window.startTime = ${state.startTime};
    ...
    function updateUptime() {
      ...
      if (el && window.startTime) el.textContent = formatUptime(Date.now() - window.startTime);
    }
  ...
  if (APP_URL) {
    setInterval(async () => {
      try {
        await require('axios').get(`${APP_URL}/health`, { timeout: 5000 });
      } catch (_) {}
    }, 15 * 60 * 1000);
  }
}
...
async function startBot() {
  try {
    ...
    if (savedData?.creds?.me?.id) {
      ...
      if (savedData.keys) Object.assign(authState.keys, savedData.keys);
      logger.info(`✅ Session restaurée (${savedData.creds.me.id})`);
    }
    ...
    sock.ev.on('connection.update', async ({ connection, lastDisconnect, qr }) => {
      ...
      if (connection === 'close') {
        ...
        logger.warn(`⚠️ Déconnecté (${statusCode}: ${reason})`);

        const isLoggedOut = statusCode === DisconnectReason.loggedOut;
        const is515 = reason.includes('515');
        ...
        if (isLoggedOut) {
          ...
          logger.info('🔄 WhatsApp 515');
        }

        if (reconnectAttempts >= MAX_RECONNECT_ATTEMPTS) {
          logger.error(`❌ Max reconnections (${MAX_RECONNECT_ATTEMPTS})`);
          process.exit(1);
        }

        const delay = calculateBackoffDelay(reconnectAttempts);
        reconnectAttempts++;
        logger.info(`🔄 Reconnexion dans ${Math.round(delay / 1000)}s (${reconnectAttempts}/${MAX_RECONNECT_ATTEMPTS})`);
    ...
    logger.error(`Erreur startBot: ${err.message}`);
    state.lastError = err.message;
    reconnectAttempts++;

    if (reconnectAttempts >= MAX_RECONNECT_ATTEMPTS) {
      logger.error(`❌ Arrêt après ${MAX_RECONNECT_ATTEMPTS} tentatives`);
      process.exit(1);
    }
